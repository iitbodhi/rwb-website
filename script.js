...
/* ── BACKGROUND MUSIC ── */
let _bgAudio = null, _bgMusicPlaying = false, _bgFadeTimer = null;
...
function _fadeVolume(audio, from, to, ms, cb) {
    ...
    const steps = 40, stepMs = ms / steps, diff = (to - from) / steps;
    let i = 0;
...
function initMilesDownload() {
    const btn = document.getElementById('miles-download-btn');
    const msg = document.getElementById('miles-coming-soon');
    if (!btn || !msg) return;
    btn.addEventListener('click', () => {
        msg.classList.toggle('active');
        if (msg.classList.contains('active')) msg.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
    });
}
