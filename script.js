...
function initNav(){
    ...
    const closeNav=()=>{toggle.classList.remove('open');right.classList.remove('open');document.body.style.overflow='';};
    const openNav=()=>{toggle.classList.add('open');right.classList.add('open');document.body.style.overflow='hidden';};
    ...
    toggle.addEventListener('click',(e)=>{e.stopPropagation();right.classList.contains('open')?closeNav():openNav();});
    links.forEach(l=>l.addEventListener('click',closeNav));
    document.querySelector('.nav-cta')?.addEventListener('click',closeNav);
    document.addEventListener('click',(e)=>{if(!right.classList.contains('open'))return;if(!right.contains(e.target)&&!toggle.contains(e.target))closeNav();});
...
/* Mobile spec row - moves tags into flex row below logo */
...
function parseAndRenderEvents(csv){
    ...
    container.innerHTML=rows.map((row,idx)=>{
        ...
        /* Build display day: single or vertical range */
...
function initLaunchMusic(){ return; /* music disabled */ }
...
function enableGallerySwipe() {
    const container = document.getElementById('gal-belt-outer');
    if (!container) return;
    ...
    container.addEventListener("touchend", (e) => {
        ...
        if (startX - endX > 50) container.scrollLeft += 300;
