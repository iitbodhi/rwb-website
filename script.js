...
function buildSpecTagsRow() {
    ...
    if (!wrap || wrap.querySelector('.spec-tags-row')) return;
    const t1 = wrap.querySelector('.st-1');
    const t2 = wrap.querySelector('.st-2');
    const t3 = wrap.querySelector('.st-3');
    if (!t1 || !t2 || !t3) return;
    const row = document.createElement('div');
    row.className = 'spec-tags-row';
    /* order: 400+ Ronins | 15+ Camps | 20+ Centers */
    [t1, t3, t2].forEach(tag => { tag.style.cssText = ''; row.appendChild(tag); });
