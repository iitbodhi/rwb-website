...
/* ================================================================
   BACKGROUND MUSIC PLAYER — music.mp3 via HTML Audio
================================================================ */
let _bgAudio = null;
...
/* 5. PRELOADER — triple-safe dismiss */
function initPreloader() {
    ...
    /* If page already fully loaded (cached), hide immediately */
    if (document.readyState === 'complete') { setTimeout(hide, 80); return; }
    /* Otherwise: hide on load, with 400ms hard cap */
...
function initNav() {
    const nav = document.getElementById('nav'), toggle = document.getElementById('nav-toggle'),
        right = document.getElementById('nav-right'), scrl = document.getElementById('hero-scroll'),
        backdrop = document.getElementById('nav-backdrop'),
        links = document.querySelectorAll('.nav-link');

    const openMenu = () => {
        toggle.classList.add('open'); right.classList.add('open');
...
function processEventCards(cards) {
    cards.forEach(card => {
        ...
        if (pct < 1) {
            /* Slot status clickable → Instagram */
            sw.style.cursor = 'pointer';
            sw.title = 'Click to join on Instagram';
            sw.addEventListener('click', () => window.open('https://www.instagram.com/ronin_warriors_bengal', '_blank'));
            /* Rolling banner → Instagram */
            ...
            b.addEventListener('click', () => window.open('https://www.instagram.com/ronin_warriors_bengal', '_blank'));
            card.appendChild(b);
        }
    });
}
...
function initAboutLogoRev() {
    ...
    logo.addEventListener('click', () => {
        try {
            _audioUnlocked = true;
            /* Duck background music while rev plays */
            ...
            hint?.classList.add('revving'); setTimeout(() => hint?.classList.remove('revving'), 3000);
        }
...
function initBadgeMusic() {
    /* Hero badge (desktop + mobile) */
    document.querySelectorAll('.hero-badge .badge-ring, .hero .badge-ring').forEach(ring => {
        ring.addEventListener('click', toggleBgMusic);
        ring.title = 'Click to play / stop music';
...
function toggleBgMusic() {
    ...
    if (_bgMusicPlaying) {
        _fadeVolume(_bgAudio, _bgAudio.volume, 0, 1200, () => {
            ...
            _setBadgeMusicClass(false);
        });
    } else {
        _bgAudio.currentTime = _bgAudio.currentTime || 0;
        _bgAudio.volume = 0;
        _bgAudio.play().then(() => {
            _bgMusicPlaying = true;
            _setBadgeMusicClass(true);
            _fadeVolume(_bgAudio, 0, 0.65, 1500);
...
function _fadeVolume(audio, from, to, ms, cb) {
    ...
    const steps = 40, stepMs = ms / steps, diff = (to - from) / steps; let i = 0;
    audio.volume = Math.max(0, Math.min(1, from));
    _bgFadeTimer = setInterval(() => {
        i++; audio.volume = Math.max(0, Math.min(1, from + diff * i));
        if (i >= steps) { clearInterval(_bgFadeTimer); audio.volume = Math.max(0, Math.min(1, to)); if (cb) cb(); }
    }, stepMs);
}

function _setBadgeMusicClass(on) {
    document.querySelectorAll('.hero-badge .badge-ring, .hero .badge-ring').forEach(r => r.classList.toggle('music-on', on));
}
