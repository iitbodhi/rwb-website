...
function initNav(){
    ...
    /* Click outside nav drawer to close (click on the dim backdrop) */
    document.addEventListener('click',(e)=>{
        if(!right.classList.contains('open'))return;
        /* If click is outside both the drawer and toggle button */
        if(!right.contains(e.target)&&!toggle.contains(e.target)){
            toggle.classList.remove('open');
            right.classList.remove('open');
            document.body.style.overflow='';
        }
...
/* ── MOBILE SPEC TAGS ROW ── */
function initMobileSpecRow(){
    if(window.innerWidth>980)return;
    const wrap=document.querySelector('.patch-wrap');
    if(!wrap||wrap.querySelector('.spec-row'))return;
    const t1=wrap.querySelector('.spec-tag-1');
    const t2=wrap.querySelector('.spec-tag-2');
    const t3=wrap.querySelector('.spec-tag-3');
    if(!t1||!t2||!t3)return;
    const row=document.createElement('div');
    row.className='spec-row';
    row.style.cssText='display:flex;gap:.5rem;justify-content:center;align-items:stretch;width:100%;flex-wrap:nowrap';
    /* Order: 400+ Ronins | 15+ Camps | 20+ Centers */
    row.appendChild(t1);row.appendChild(t3);row.appendChild(t2);
