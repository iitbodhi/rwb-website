...
document.addEventListener('DOMContentLoaded',()=>{
    /* Preloader FIRST — guarantees it always dismisses */
    initPreloader();
    try{
        buildGrain();
        initThemeToggle();
        ...
        initAboutLogoRev();
        initLightbox();
    }catch(e){ console.warn('RWB init error:',e); }
});
...
/* 3. CURSOR — disabled per user request, normal cursor used */
function initCursor(){
...
/* ── MOBILE SPEC ROW ──
   On mobile, wraps the 3 spec tags into a horizontal flex row below the logo.
   The spec tags use absolute positioning relative to .patch on desktop.
   On mobile we override them to be in a flex row via this wrapper.
*/
function initMobileSpecRow(){
    ...
    /* Override absolute positioning on each tag */
    [t1, t3, t2].forEach(tag => {
...
/* 17. MUSIC — removed per user request */
function initLaunchMusic(){ /* disabled */ }
